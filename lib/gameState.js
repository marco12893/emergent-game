...
import { sanitizeCustomMap } from '@/lib/customMap'
import { createMap4ObjectiveState } from '@/game/map4Objectives'
...
// Create a new game
const createGame = async (gameId, mapId = DEFAULT_MAP_ID, isWinter = false, teamMode = false, customMap = null) => {
  const sanitizedCustomMap = sanitizeCustomMap(customMap)
  const mapConfig = sanitizedCustomMap || getMapConfig(mapId)
  const generatedMapData = sanitizedCustomMap || generateMapData(mapId)
  const { hexes, terrainMap, tileMap, deploymentZones } = generatedMapData
  ...
  const game = {
    ...
    tileMap: sanitizedCustomMap?.tileMap || tileMap || null,
    deploymentZones: sanitizedCustomMap?.deploymentZones || deploymentZones || null,
    isWinter,
    teamMode,
    ...
    waitlist: [],
    playersReady,
    aiDeploymentUnitCount: 5,
    aiDeploymentMode: 'auto',
    aiDeploymentComposition: { SWORDSMAN: 1, ARCHER: 1, KNIGHT: 1, MILITIA: 1, CATAPULT: 1, WAR_GALLEY: 0 },
    log: ['Lobby open! Assemble your team and prepare for battle.'],
    chatMessages: [],
    inactivePlayers: [],
    objectiveControl: { [attackerId]: 0, [defenderId]: 0 },
    map4ObjectiveState: mapConfig.id === 'MAP_4' ? createMap4ObjectiveState({ terrainMap, teamMode }) : null,
    battleStats: null,
    turnStartedAt: null,
