...
/**
 * Sanitize participant ID used for lobby kick targets (players or spectators)
 * @param {string} participantID - Participant identifier
 * @returns {string|null} - Sanitized identifier or null if invalid
 */
export function sanitizeParticipantID(participantID) {
  if (!participantID || typeof participantID !== 'string') {
    return null
  }

  const sanitized = participantID.trim()
...
/**
 * Sanitize reconnect key used to restore a player's slot after refresh/reconnect
 * @param {string} reconnectKey - Stable reconnect identifier stored on client
 * @returns {string} - Sanitized reconnect key or empty string when invalid
 */
export function sanitizeReconnectKey(reconnectKey) {
  if (!reconnectKey || typeof reconnectKey !== 'string') {
    return ''
  }

  const sanitized = reconnectKey.trim()
...
export function sanitizeAction(action) {
  ...
  const allowedActions = [
    'placeUnit', 'removeUnit', 'moveUnit', 'undoMove', 'attackUnit', 'attackTerrain', 'endTurn', 'readyForBattle',
    'selectUnit', 'deselectUnit', 'toggleRetreatMode', 'retreatUnit', 'sendChat',
    'claimSlot', 'startBattle', 'setFogOfWar', 'setWinterMode', 'setTeamMode', 'setAiDeploymentUnitCount', 'setAiDeploymentComposition', 'kickParticipant', 'moveParticipant',
    'addAiPlayer'
  ]
...
export function sanitizeMapId(mapId) {
  ...
  const allowedMapIds = ['MAP_1', 'MAP_2', 'MAP_3', 'MAP_4', 'CUSTOM']
